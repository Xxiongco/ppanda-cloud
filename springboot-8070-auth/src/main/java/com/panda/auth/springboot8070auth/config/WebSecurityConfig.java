package com.panda.auth.springboot8070auth.config;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
				//.httpBasic().and()// 开启basic验证 获取 token 时需要带basic的授权
				// 同时需要实现 ClientDetailsService
				.authorizeRequests()
				// 放行
				.antMatchers("/oauth/**")
				.permitAll()
...
//                .successForwardUrl("/login-success") //自定义登录成功跳转页
	...
	protected UserDetailsService userDetailsService() {
		...
		userDetailsService.createUser(User.withUsername("user_1").
